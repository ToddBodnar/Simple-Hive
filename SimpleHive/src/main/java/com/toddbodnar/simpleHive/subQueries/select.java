...
public class select extends query<Text, Object> {
    ...
    /**
     *
     * @param toRetain A string of comma seperated values to retain, either
     * _coln, for column n, or an object, or "x as y" for an object x called y
     */
    public select(String toRetain) {
        query = toRetain;

        /**
         *  //this code assumes a preparsed variable list, moved this down to
         * the setInput command to have a JIT processing String split[] =
         * toRetain.split(","); variable = new boolean[split.length]; value =
         * new Object[split.length]; names = new String[split.length]; for(int
         * ct=0;ct<split.length;ct++) { if(split[ct].contains(" as ")) { int
         * start = -1; for(int i=0;i<split[ct].length();i++) {
         * if(split[ct].substring(i).startsWith(" as ")) { start = i; break; } }
         * names[ct]=split[ct].substring(start+4); split[ct] =
         * split[ct].substring(0, start); } if(split[ct].startsWith("_col")) {
         * variable[ct] = true; value[ct] =
         * Integer.parseInt(split[ct].substring(4)); } else { variable[ct] =
         * false; value[ct] = split[ct]; } }*
         */
    }
